...
class PendingEdge:
    ...
    def __repr__(self):
        return (f"PendingEdge(id={self.edge_id}, label='{self.label}', source={self.source}, target={self.target})")

@dataclass
class PendingVariable:
    variable_node_id: int = 0
    abstracted_nodes: list[int] = field(default_factory=list, repr=False)
